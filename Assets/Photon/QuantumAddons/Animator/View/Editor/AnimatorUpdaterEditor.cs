namespace Quantum.Addons.Animator
{
  ...
  [CustomEditor(typeof(IAnimatorEntityViewComponent))]
  public unsafe class AnimatorUpdaterEditor : Editor
  {
    public bool EnableDebug = true;

    protected QuantumEntityViewComponent _animatorUpdater;

    void OnEnable()
    {
      _animatorUpdater = (QuantumEntityViewComponent)target;
    }
    ...
    public override void OnInspectorGUI()
    {
      ...
      if (_animatorUpdater.Game == null)
      {
        GUILayout.Label("Debug values will be updated on the Quantum session has started.");
        return;
      }

      Frame frame = _animatorUpdater.PredictedFrame;
      var animator = frame.Get<AnimatorComponent>(_animatorUpdater.EntityRef);
      ...
      for (int layerIndex = 0; layerIndex < animatorGraph.Layers.Length; layerIndex++)
      {
        ...
        var layerData = layers[layerIndex];
        var blendTreeWeights = frame.ResolveDictionary(layers[layerIndex].BlendTreeWeights);
        EditorGUILayout.BeginVertical("box");
        ...
        for (int stateIndex = 0; stateIndex < layer.States.Length; stateIndex++)
        {
          ...
          float fillAmount = 0;
          if (layer.IsStateActive(&layerData, state))
          {
            fillAmount = (layerData.Time / layerData.Length).AsFloat;
