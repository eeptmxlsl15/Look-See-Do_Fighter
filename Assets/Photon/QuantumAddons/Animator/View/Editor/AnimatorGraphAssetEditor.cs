...
namespace Quantum.Addons.Animator
{
  ...
  public class AnimatorGraphAssetEditor : Editor
  {
    ...
    private bool _showDetailedData = true;
    ...
    public override void OnInspectorGUI()
    {
      ...
      if (GUILayout.Button("Bake Animator Graph"))
      {
        AnimatorGraphBaker.OnBake(graph);
      }

      _showDetailedData = EditorGUILayout.BeginFoldoutHeaderGroup(_showDetailedData, "Detailed Info");
      if (_showDetailedData)
      {
        if (graph != null && graph.IsValid && graph.Layers != null)
        {
          ...
          if (layerCount > 0)
          {
            ...
            var currentLayer = GUILayout.Toolbar(_selectedLayer, layerNames);
            if (currentLayer != _selectedLayer)
            {
              //Set the selected state to zero case the layer has changed
              _selectedState = 0;
            }

            _selectedLayer = currentLayer;
            ...
            if (layer != null)
            {
              ...
              if (stateCount > 0)
              {
                ...
                if (state != null)
                {
                  ...
                  if (_showTransitions = EditorGUILayout.Foldout(_showTransitions, "Transitions"))
                  {
                    var originalBackgroundColor = GUI.backgroundColor;
                    GUI.backgroundColor = Color.blue * 1;
                    ...
                    for (int transitionIndex = 0; transitionIndex < transitionCount; transitionIndex++)
                    {
                      ...
                      for (int conditionIndex = 0; conditionIndex < conditionCount; conditionIndex++)
                      {
                        ...
                        GUI.contentColor = Color.white * 2; // Adjust the multiplier to make it darker or lighter
        ...
        else
        {
          ...
          GUILayout.Label("This is not a valid AnimatorGraph,  fix and import again!", errorStyle,
            GUILayout.Height(50));
    ...
    private void AnimationClipGui(AnimatorClip clip)
    {
      ...
      EditorGUILayout.LabelField(string.Format("LoopTime: {0}", clip.Data.LoopTime));
      ...
      if (_showMovementData = EditorGUILayout.Foldout(_showMovementData, "Movement Data"))
      {
        ...
        for (int f = 0; f < frameCount; f++)
          EditorGUILayout.LabelField(positions[f].ToString("F3"), GUILayout.Width(160));
        EditorGUILayout.EndVertical();
