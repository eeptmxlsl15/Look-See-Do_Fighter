namespace Quantum.Addons.Animator
{
  ...
  public unsafe interface IAnimatorMotion
  {
    ...
    void ProcessEvents(Frame f, LayerData* layerData, FP currentTime)
    {
    }

    void GenerateBlendList(Frame f, LayerData* layerData, AnimatorLayer layer, AnimatorState state, FP weight,
      List<AnimatorRuntimeBlendData> list);

    FP CalculateLength(Frame f, LayerData* layerData, FP weight, AnimatorState state);
    void CalculateWeights(Frame f, AnimatorComponent* animatorComponent, LayerData* layerData, int stateId);
    bool CalculateSpeed(Frame f, AnimatorComponent* animatorComponent, LayerData* layerData, out FP speed);
  }
  ...
  public abstract unsafe class AnimatorMotion : IAnimatorMotion
  {
    ...
    public virtual void ProcessEvents(Frame f, AnimatorComponent* animator, LayerData* layerData, int stateId)
    {
    }

    public virtual void GenerateBlendList(Frame f, LayerData* layerData,
      AnimatorLayer layer,
      AnimatorState state, FP weight, List<AnimatorRuntimeBlendData> list)
    {
    }

    public virtual FP CalculateLength(Frame f, LayerData* layerData, FP weight,
      AnimatorState state)
    {
      return FP._0;
    }

    public virtual void CalculateWeights(Frame f, AnimatorComponent* animatorComponent, LayerData* layerData,
      int stateId)
    {
    }

    public virtual bool CalculateSpeed(Frame f, AnimatorComponent* animatorComponent, LayerData* layerData,
      out FP speed)
    {
      speed = FP._1;
      return false;
  ...
  public unsafe class AnimatorClip : AnimatorMotion
  {
    ...
    public override bool LoopTime => Data.LoopTime;

    public string ClipName => Data != null ? Data.ClipName : "";

    public override int TreeIndex { get; set; }

    public override void ProcessEvents(Frame f, AnimatorComponent* animator, LayerData* layerData, int stateId)
    {
      if (Data.Events == null)
      {
        return;
      ...
      {
        AnimatorEventAsset eventAsset = f.FindAsset(animatorEvent.AssetRef);
        if (eventAsset.IsMuted == false)
        {
          animatorEvent.Evaluate(f, animator, layerData);
        }
      }
    }

    public override void GenerateBlendList(Frame f, LayerData* layerData,
      AnimatorLayer layer,
      AnimatorState state, FP weight, List<AnimatorRuntimeBlendData> list)
    {
      FP time = FP._0;
      ...
      if (state.Id == layerData->CurrentStateId)
      {
        time = layerData->Time;
        lastTime = layerData->LastTime;
        normalizedTime = layerData->NormalizedTime;
        calculatedLength = layerData->Length;
      }

      if (state.Id == layerData->FromStateId)
      {
        time = layerData->FromStateTime;
        lastTime = layerData->FromStateLastTime;
        normalizedTime = layerData->FromStateNormalizedTime;
        calculatedLength = layerData->FromLength;
      }

      if (state.Id == layerData->ToStateId)
      {
        time = layerData->ToStateTime;
        lastTime = layerData->ToStateLastTime;
        normalizedTime = layerData->ToStateNormalizedTime;
        calculatedLength = layerData->ToLength;
      }

      if (layerData->ToStateId != 0)
      {
        // transition is happening
        FP transitionWeight = FPMath.Clamp(layerData->TransitionTime / layerData->TransitionDuration, FP._0, FP._1);
        if (state.Id == layerData->FromStateId) transitionWeight = FP._1 - transitionWeight;
    ...
    public override FP CalculateLength(Frame f, LayerData* layerData, FP weight,
      AnimatorState state)
    {
      if (Data == null)
        return 0;
    ...
    public override void CalculateWeights(Frame f, AnimatorComponent* animatorComponent, LayerData* layerData,
      int stateId)
    {
      //not implemented
    }

    public override bool CalculateSpeed(Frame f, AnimatorComponent* animatorComponent, LayerData* layerData,
      out FP speed)
    {
      speed = FP._1;
      return false;
  ...
  public unsafe class AnimatorBlendTree : AnimatorMotion
  {
    ...
    public override bool LoopTime => true;

    public override int TreeIndex { get; set; }

    public override void ProcessEvents(Frame f, AnimatorComponent* animator, LayerData* layerData, int stateId)
    {
      var weights = AnimatorComponent.GetStateWeights(f, layerData, stateId);
      for (int i = 0; i < Motions.Length; i++)
      {
        var clipWeight = weights[i];
        if (clipWeight > 0)
        {
          Motions[i].ProcessEvents(f, animator, layerData, stateId);
        }
      }
    }
    ...
    {
      FP zf = FP._0_05;
      var weights = AnimatorComponent.GetStateWeights(f, layerData, state.Id);
      for (int i = 0; i < MotionCount; i++)
        if (weights[i] > zf)
          Motions[i].GenerateBlendList(f, layerData, layer, state, weights[i], list);
    }
      ...
      AnimatorState state)
    {
      ...
      for (int i = 0; i < MotionCount; i++)
        if (Motions[i] != null && weights[i] > zf)
          output += Motions[i].CalculateLength(f, layerData, weights[i], state);
      return output;
    }
      ...
      int stateId)
    {
      ...
      if (Is2D())
      {
        ...
        FP blendParameterX = FPMath.Clamp(
          *AnimatorComponent.Variable(f, animatorComponent, BlendParameterIndex)->FPValue,
          -FP._1, FP._1);
        FP blendParameterY = FPMath.Clamp(
          *AnimatorComponent.Variable(f, animatorComponent, BlendParameterIndexY)->FPValue,
          -FP._1, FP._1);
      ...
      else
      {
        ...
        var blendVariable = FPMath.Clamp(
          *AnimatorComponent.Variable(f, animatorComponent, BlendParameterIndex)->FPValue,
    ...
    private FP[] CalculateWeight_1D(FP targetPoint, FPVector2[] points)
    {
      ...
      for (int pointsIndex = 0; pointsIndex < points.Length; pointsIndex++)
      {
        ...
        if (targetPointDistance > pointAmplitude || pointAmplitude == 0)
        {
